$(document).ready(function () {
	...
	$(document).off('getBlogPageListResponse').on('getBlogPageListResponse', function (e, data, status) {	
		var blogList = data.records;
		var content='';
		...
		$.each(blogList, function(index) {
			...
			content += `
				<section id="intro" class="section intro no-padding blog-section">
	...
	$(document).off('getAllQualificationAPIResponse').on('getAllQualificationAPIResponse', function (e, data, status) {			
		...
		$.each( userList, function(index) {
			...
			$.ajax({
				...
				'global': false,
				'dataType': 'json',
				'url': "http://tf.1ppl.me/api/user/"+userId+"/qualifications/?all=true",		
			...
			<div class='container profile-basic-section'>
				<div class='profile-body-section'> 
					...
					<div class='col-md-10 profile-pic-section'>
					  <div class='avatar profile-pic-specific'> <img src='http://tf.1ppl.me/api/user/`+ userList.id +`/avatar.medium.jpg' alt='' class='img-responsive profilePicture'> </div>
			...
			<div class="tab-content col-md-12">
			  ...
			  <div id="profile" class="tab-pane fade">
				...
				var subjectList = $.ajax({
					url: 'http://tf.1ppl.me/api/user/'+userList.id + '/subject',
					async: false,
					dataType: 'json'
				}).responseJSON;					
				...
				<div class="col-md-12 form-group tutorSection">
					...
					{
						var qList = $.ajax({
							url: 'http://tf.1ppl.me/api/user/'+userList.id +"/qualifications/?all=true",
							async: false,
							dataType: 'json'
						}).responseJSON;							
	...
	$(document).off('searchUserListResponse').on('searchUserListResponse', function (e, data, status, role ) {	
		console.log(data);		
		var userList = data.records;
		...
		{
			$('.SearchResultList').append("<label class='noRecord'> No record found.</label>");
		}
		$.each(userList, function(index) {
			var listData = "<a target='_blank' href='http://tf.1ppl.me/profile.html?username="+userList[index].username+"'><div class='search-result'><div class='container'>";
			var fullName =  userList[index].first_name +  " " +  userList[index].last_name;
			
			var userProfile = $.ajax({
						url: 'http://tf.1ppl.me/api/user/'+userList[index].id+'?data=id,username,email,first_name,last_name,gender,role&status=joint_at,last_login,violation,active,email_verified,total_login&profile=address,course,level,description',
						async: false,
						dataType: 'json'
			}).responseJSON;
			
			var userSubject =  $.ajax({
						url: 'http://tf.1ppl.me/api/user/'+userList[index].id+'/subject',
						async: false,
						dataType: 'json'
			}).responseJSON;
			
			var userQualification =  $.ajax({
						url: 'http://tf.1ppl.me/api/user/'+userList[index].id+'/qualifications/?all=true',
						async: false,
						dataType: 'json'
			}).responseJSON;
			...
			if(userProfile.profile)
			{
				...
				listData += "<div class='result-pic'><img src='http://tf.1ppl.me/api/user/"+ userList[index].id +"/avatar.small.jpg'/>";
				listData += "<button class='search-result-button'>View profile </button></div>";  
				listData += "<div class='result-profile'> ";  
				listData += "<h4> " + fullName + "</h4>";  
				listData += "<div class='col-md-12 profile-row profile-head'><div class='col-md-4 profile-row'> Location </div>";  
				listData += "<div class='col-md-8'>"+userProfile.profile.address.address+"  </div> </div>";  
				
				//Education level and qualification 
				if(role =='student')
				{
					listData += "<div class='col-md-12 profile-row profile-head'><div class='col-md-4 profile-row'>Education level</div>";  
					listData += "<div class='col-md-8'>"+getLevelOptionByID(userProfile.profile.course)+"  </div> </div>";  
				}
				else 
				{
					listData += "<div class='col-md-12 profile-row profile-head'><div class='col-md-4 profile-row'> Qualification </div>";  
				...
				listData += "<div class='col-md-12 profile-row profile-head'><div class='col-md-4 profile-row'> Description </div>";  
				listData += "<div class='col-md-8'>"+userProfile.profile.description+"</div> </div>";    
				listData += "</div>";  
				listData += "<div class='result-detail'>  <h5 class='roleSubject'></h5>";  
				listData += "<div class='profile-row'>";
				
				//Subject list 
				for(var c = 0; c < userSubject.length; c++)
				{
					listData +="<div class='subjectList col-md-12 profile-row'> <div class='subjectRow col-md-12 '><div class='col-md-4 profile-row'>Course  </div>  <div class='col-md-8'>"+getCourseOptionByID(userSubject[c].course) +" </div> </div><div class='subjectRow col-md-12'><div class='col-md-4 profile-row'> Level </div>  <div class='col-md-8'> "+getLevelOptionByID(userSubject[c].level)+"</div></div></div>";
				}		
		...
		if(role=="student")
		{			
			$('.roleSubject').html('Interested subject');
		}
		else 
		{			
			$('.roleSubject').html('Teaching subject');
		}
		$('.loader').hide();
	...
	function getUsername(userId)
	{
		...
		$.ajax({
			...
			'global': false,
			'dataType': 'json',
			'url': "http://tf.1ppl.me/api/user/" + userId,		
			'success': function (data) {
				dataContent = data;
			}		
		});
	...
	function getCourseOptionByID(id)
	{
		...
		$.ajax({
			...
			'global': false,
			'dataType': 'json',
			'url': "http://tf.1ppl.me/api/system/course/" + id,		
			'success': function (data) {
				dataContent = data;
			}		
		});
	...
	function getLevelOptionByID(id)
	{
		...
		$.ajax({
			...
			'global': false,
			'dataType': 'json',
			'url': "http://tf.1ppl.me/api/system/level/" + id,		
			'success': function (data) {
				dataContent = data;
			}		
		});		
	...
	function getStateOptionByID(id)
	{
		...
		$.ajax({
			...
			'global': false,
			'dataType': 'json',
			'url': "http://tf.1ppl.me/api/system/states/code.json",	
			'success': function (data) {
				dataContent = data;
			}		
		});		

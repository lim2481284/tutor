  ...
  $(document).off('getBlogPageListResponse').on('getBlogPageListResponse', function (e, data, status, totalRecord, pageNumber) {
    ...
    if(pageNumber >pageLimit)
    {
              ...
              for(var i=1; i<=stopPage;i++)
              {
                if(pageNumber == i )
                {
                  ...
                  $.each(blogList, function(index) {
                    ...
                    <div class='blogContent col-sm-12'>
                    `+$('<div>' + blogContent + '</div>').text()+`
                    </div>
                    <div class='blogTag col-sm-12'>

                    `;

                    $.each(tagList, function(index) {
                      content +="<div class='blogTagItem'>"+tagList[index]+"</div>	";
                    });

                    content +=`
                    </div>
                    <div class='blogFooter col-sm-12'>
                    <div class='blogFooterLeft'>
                    <button class='readMoreBtn'> Read more </button>
                    </div>
                    <div class='blogFooterRight'>
                    Author : `+getUsername(blogList[index].author)+`, `+formattedDate+`
                    </div>
                    </div>
                    </div>
                    </a>
      ...
      $(document).off('getSpecificUserAPIResponse').on('getSpecificUserAPIResponse', function (e, data, status,xhr) {
        ...
        var img_url = tutorAPI.url+"user/"+ userList.id +"/avatar.medium.jpg";
        var listData = `
        <div class='container profile-basic-section'>
        <div class='profile-body-section'>
        <div class='col-md-1'></div>
        <div class='col-md-10 profile-pic-section'>
        <div class='avatar profile-pic-specific'> <img src='`+img_url+`' alt='' class='img-responsive profilePicture'> </div>
                          ...
                          else
                          {
                            ...
                            for(var i=1; i<=stopPage;i++)
                            {
                              if(pageNumber == i )
                              {
                                ...
                                //Generate search result
                                $.each(userList, function(index) {
                                  var listData = "<a class='result_link' target='_blank' href='http://tf.1ppl.me/profile.html?username="+userList[index].username+"'><div class='search-result'><div class='container searchResultContainer'>";
                                  var fullName =  userList[index].first_name +  " " +  userList[index].last_name;
                                  var test = '';
                                  $.ajax({
                                    url: tutorAPI.url+'user/'+userList[index].id+'?data=id,username,email,first_name,last_name,gender,role&status=joint_at,last_login,violation,active,email_verified,total_login&profile=address,course,level,description',
                                        ...
                                        success: function(response_2){
                                          ...
                                          $.ajax({
                                            ...
                                            success: function(response_3){
                                              ...
                                              var img = tutorAPI.url + "user/" +userList[index].id +"/avatar.big.jpg";
                                              listData += `
                                              <div class='result-pic'><img src='`+img+`'/>
                                              </div>
                                              <div class='result-profile-box'>
                                              <img src='assets/images/leftArrow.png' class='result-left-arrow'/>
                                              <div class='result-profile'>
                                              <h4> ` + fullName + `</h4>
                                              <div class='col-md-12 profile-row profile-head noPaddingLeft'>
                                              <div class='col-md-3 profile-row resultLabel'>
                                              Location
                                              </div>
                                              <div class='col-md-9 noPaddingLeft descriptionRow'>
                                              `+profileAddress+`
                                              </div>
                                              </div>
                                            `;
                                            ...
                                            */
                                            ...
                                            <div class='readMoreLabel'>Read more</div>
                                            </div>
                                            </a>
                                            `;
